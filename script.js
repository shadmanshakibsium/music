...
function loadArtistView() {
    ...
    sortedArtists.forEach(artist => {
        ...
        artistTitle.classList.add('playlist-title');
        ...
        artistSongsEl.classList.add('playlist-songs');
...
function loadplaylistView() {
    ...
    const githubFetches = types.map(e =>
        fetch(`${GITHUB_BASE}data/${e}.json`)
            .then(r => r.ok ? r.json() : [])
            .then(t => t.map(t => ({
                ...t, folder: e,
                uid: generateUID(`github_${e}/${t.file}`),
                src: `${GITHUB_BASE}songs/${e}/${t.file}`
            })))
            .catch(() => [])
    );

    const localFetches = types.map(e =>
        fetch(`data/${e}.json`)
            .then(r => r.ok ? r.json() : [])
            .then(t => t.map(t => ({
                ...t, folder: e,
                uid: generateUID(`local_${e}/${t.file}`),
                src: `songs/${e}/${t.file}`
            })))
            .catch(() => [])
    );

    Promise.all([...githubFetches, ...localFetches]).then(results => {
        const seen = new Set();
        const allSongs = results.flat().filter(s => !seen.has(s.uid) && seen.add(s.uid));

        const n = {};
        allSongs.forEach(e => {
            if (!e.playlist) return;
            e.playlist.split(",").map(e => e.trim().toLowerCase()).filter(e => e).forEach(t => {
                n[t] || (n[t] = []);
                n[t].push(e);
            });
        });
        ...
        Object.keys(n).sort().forEach(e => {
            ...
            t.classList.add("playlist-title");
            t.textContent = e.toUpperCase();
            playlistListEl.appendChild(t);

            const s = document.createElement("ul");
            s.classList.add("playlist-songs");
            s.style.display = "none";
            playlistListEl.appendChild(s);

            t.addEventListener("click", () => {
                s.style.display = "block" === s.style.display ? "none" : "block";
            });

            n[e].sort((e, t) => e.name.localeCompare(t.name)).forEach((t, l) => {
                const a = document.createElement("li");
                a.classList.add("music-item");
                a.setAttribute("data-uid", t.uid);
                a.innerHTML = `
          <div class="info"><span class="title">${t.name}</span></div>
          <button class="add-queue-btn" title="Add to Queue"><i class="fas fa-plus"></i></button>`;

                a.querySelector(".add-queue-btn").addEventListener("click", e => {
                    e.stopPropagation();
                    addToQueue(t);
                });

                setupLongPressForItem(a, t);

                t.uid === currentSongUID && a.classList.add("playing");
                ...
                a.addEventListener("click", t => {
                    ...
                    currentIndex = l;
                    playSong(l);
                    s.querySelectorAll(".music-item").forEach(e => e.classList.remove("playing"));
...
fsCloseBtn.addEventListener('click', () => {
    ...
    setTimeout(() => {
        ...
        if (currentView === 'all') {
            ...
            if (currentSongUID) {
                const allPlaylistSongs = document.querySelectorAll('.playlist-songs .music-item');


                document.querySelectorAll('.playlist-songs').forEach(el => {
                ...
                playlistTitles.forEach(title => {
                    const songsEl = title.nextElementSibling;
                    if (!songsEl || !songsEl.classList.contains('playlist-songs')) return;

                    const targetItem = songsEl.querySelector(`.music-item[data-uid="${currentSongUID}"]`);
                    if (targetItem) {
                        songsEl.style.display = 'block';


                        allPlaylistSongs.forEach(el => el.classList.remove('playing'));
...
const viewOrder = ['all', 'folders', 'playlist', 'artist'];
    ...
    tab.addEventListener('click', () => {
        ...
        setTimeout(() => {
            ...
            if (!viewLoaded[currentView]) {
                ...
                else if (currentView === 'playlist' && playlistListEl.innerHTML === '') loadplaylistView();
            }
...
(function () {
    ...
    function makeNormal() {
        ...
        playlistListEl.style.paddingTop = '';
